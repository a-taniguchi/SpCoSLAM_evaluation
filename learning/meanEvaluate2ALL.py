#c-od-i-n-:-ut-f---8
#!/usr/bin/env python
#PAR_w_MI ga PARs tosite hozon sareteita mono wo syuusei.
#Akira Taniguchi 
#-2018/01/25
import sys
import string
...
fp = open(datafolder + '/Evaluation/' + trialname + '_' + data_num1 + '_' + data_num2 + '_Evaluation2ALL.csv', 'w')
fp.write('NMIc,NMIi,PARwMI,PARw,L,K,ESEG,PARs\n')
...
for s in range(N):
  i = 0
  for line in open(datafolder + trialname + str(s+1).zfill(3) + '/' + trialname + str(s+1).zfill(3) +'_meanEvaluation2MI.csv', 'r'): #ALL
...
for iteration in xrange(len(NMIc_MM)):
  fp.write( str(NMIc_MM[iteration])+','+ str(NMIi_MM[iteration])+','+ str(PARs_MM[iteration])+','+str(PARw_MM[iteration])+ ','+str(L_MM[iteration])+','+ str(K_MM[iteration])+','+str(ESEG_MM[iteration])+','+ str(PARss_MM[iteration]) )
  fp.write('\n')
fp.write('\n')
...
for iteration in xrange(len(NMIc_MM)):
  ...
  fp.write( str(np.std(NMIc_MS, ddof=1))+','+ str(np.std(NMIi_MS, ddof=1))+','+ str(np.std(PARs_MS, ddof=1))+','+str(np.std(PARw_MS, ddof=1)) +','+str(np.std(L_MS, ddof=1)) +','+str(np.std(K_MS, ddof=1)) +','+str(np.std(ESEG_MS, ddof=1))+','+ str(np.std(PARss_MS, ddof=1)) )
  fp.write('\n')
#np.std
#float(NMIi_M / N)
#print "NMIi mean"
