...
step = 50
DATA = ['alg1','alg2wicWS','alg2wicWSLAG10lln','alg2wicWSLAG10LM']
#['(A) SpCoSLAM','(B1) SpCoSLAM 2.0 (LAG:1)','(B2) SpCoSLAM 2.0 (LAG:10)','(B3) SpCoSLAM 2.0 (LAG:20)']
...
learningdata = ['alg1','alg2wicWS','alg2wicWSLAG10lln','alg2wicWSLAG10LM']
#["alg2wicWS","alg2wicWSLAG1LM","alg2wicWSLAG10LM","alg2wicWSLAG20LM"]
...
for d in xrange(len(DATA)):
  ...
  for s in range(N):
    ...
    i = 0
    for line in open(datafolder + trialname + str(s+1).zfill(3) + '/' + trialname + str(s+1).zfill(3) +'_EvaluationPRR2n1.csv', 'r'): #_meanEvaluationPRR2.csv
      itemList = line[:-1].split(',')
      if (i != 0) and (itemList[0] != '') and (i <= step):
