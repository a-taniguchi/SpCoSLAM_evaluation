...
for line in open(datafolder + trialname + '/'+ str(step) + '/phi' + str(particle) + '.csv', 'r'):
        ...
        for i in xrange(len(itemList)):
              ## if It.count(it) >= 1:
              if itemList[i] != '':
                #print c,i,itemList[i],ip
                phi[i][c] = float(itemList[i])
                #print W[i][c]
                #print itemList
                ip = ip + 1
              ##it = it + 1
...
#bottom_scale = 1 - (K/12.0)*0.01 #* 12.0 #*0.1 *log(1.0*(K-12)^2)
plt.subplots_adjust(left=0.60, bottom=0.70, right=0.85, top=0.95, wspace=None, hspace=None)
...
MIN = max(min([min(phi[i]) for i in range(len(phi))]),0)
#c = plt.pcolor(phi,cmap=plt.cm.hot,vmin=MIN,vmax=MAX) #gray
c = plt.pcolor(phi,cmap=plt.cm.hot,vmin=0.0,vmax=1.0) #gray
#title('')
#xlabel('Location Concepts ID', fontsize=24)
plt.xlabel('$C_t=l$', fontsize=10)
plt.ylabel('$i_t=k$', fontsize=10)


cbar=plt.colorbar(pad=0.08,shrink=0.95,aspect=12,ticks=[0.0,0.5,1.0]) #
