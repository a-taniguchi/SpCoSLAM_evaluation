#!/usr/bin/env python
# -*- coding:utf-8 -*-
#地図と場所概念を自動で可視化、保存(リアルタイムで描画してくれるようにしたかったができてない)
#Akira Taniguchi 2017/02/28-2018/02/27
import sys
import os
import random
import string
...
endstep = 50
timespeed = 1.0 #2回実行するため2で現実時間と一緒(→しないので一緒じゃない)
...
differenttime = [0.0 for in range(len(teachingtime))]
for i in range(len(teachingtime)):
  if (i+1 < 50):
    differenttime[i] = teachingtime[i+1] - teachingtime[i]
"""
#s=0
...
for line in open ( csvname , 'r'):
      ...
      list_m_count.append(int(itemList[0]))
      #s += 1
...
end_m_count = 0 #len(maplist)
#m_count = 0  #m_countの数
#m_countのindexは1から始まる
while (os.path.exists( datafolder + trialname + "/particle/" + str(end_m_count+1) + ".csv" ) == True):
    end_m_count += 1

#clocktime = []
#for m in range(end_m_count):
#  for line in open ( datafolder + trialname + "/map/map"+str(m+1)+"_clocktime.txt" , 'r'):
#      itemList = line[:-1].split(',')
#      #print itemList
#      clocktime.append(float(itemList[0]))
#      #s += 1

#differenttime = [0.0 for i in range(len(clocktime))]
#for i in range(end_m_count):
#  if (i+1 < end_m_count):
#    differenttime[i] = clocktime[i+1] - clocktime[i]
...
for m in range(1,end_m_count+1):#5,16):#5,7):#
  ...
  #time.sleep(differenttime[m-1]/timespeed)  #mapを保存した時刻経過の差分と一致させる
  for s in range(len(list)):
   if(list[s][0] == m):
    #if (m in list_m_count):
    ###new_place_draw_online.pyを実行（trailname と stepを指定）
    if (s < 1): 
      time.sleep((s)*timespeed)
    else:
  ...
  #delay = float(teachingtime[m]) - float()
  #time.sleep(differenttime[m-1]/timespeed)  #mapを保存した時刻経過の差分と一致させる
